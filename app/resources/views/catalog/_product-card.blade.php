...
<article class="sc-card flex flex-col overflow-hidden transition-transform hover:-translate-y-0.5 {{ $product->status === 'inactive' ? 'opacity-60' : '' }}"
    ...
    <a href="{{ route('catalogo.show', $product->slug) }}" class="block relative aspect-square overflow-hidden rounded-t-lg">
        ...
        @else
            ...
            <div class="w-full h-full bg-bg-muted flex flex-col items-center justify-center gap-2">
        ...
        @if($product->status === 'inactive')
            <div class="absolute top-3 right-3 bg-bg-subtle text-text-secondary text-xs font-medium px-2.5 py-1 rounded-full">
                Agotado
            </div>
        @endif
    </a>
    ...
    <div class="p-5 flex flex-col flex-1 gap-1.5">
        ...
        <a href="{{ route('catalogo.show', $product->slug) }}"
           class="text-lg font-semibold text-text-primary leading-snug hover:text-brand-primary transition-colors line-clamp-1">
            {{ $product->name }}
        </a>

        {{-- Description (body-sm, max 2 lines) --}}
        ...
        @if($product->status === 'active')
            <button type="button" class="sc-btn sc-btn-primary w-full mt-2" data-product-id="{{ $product->id }}">
